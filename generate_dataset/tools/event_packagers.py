...
class hdf5_packager_multiscale(packager):
    """
    This class packages data to hdf5 files
    """
    def __init__(self, output_path, max_buffer_size=1000000):
        ...
        self.ori_event_ps = self.events_file.create_dataset("ori_events/ps", (0, ), dtype=np.dtype(np.int8), maxshape=(None, ), chunks=True)
        ...
        self.down2_event_ps = self.events_file.create_dataset("down2_events/ps", (0, ), dtype=np.dtype(np.int8), maxshape=(None, ), chunks=True)
        ...
        self.down4_event_ps = self.events_file.create_dataset("down4_events/ps", (0, ), dtype=np.dtype(np.int8), maxshape=(None, ), chunks=True)
        ...
        self.down8_event_ps = self.events_file.create_dataset("down8_events/ps", (0, ), dtype=np.dtype(np.int8), maxshape=(None, ), chunks=True)
    ...
    def package_image(self, type, image, timestamp, img_idx, exposure_begin_t=None, exposure_end_t=None):
        image_dset = self.events_file.create_dataset("{}_images/image{:09d}".format(type, img_idx),
                data=image, dtype=np.dtype(np.uint8))
        image_dset.attrs['size'] = image.shape
        image_dset.attrs['timestamp'] = timestamp
        image_dset.attrs['type'] = "greyscale" if image.shape[-1] == 1 or len(image.shape) == 2 else "color_bgr"
        if exposure_begin_t is not None:
            image_dset.attrs['exposure_begin_t'] = exposure_begin_t
        if exposure_end_t is not None:
            image_dset.attrs['exposure_end_t'] = exposure_end_t
    ...
    def add_event_indices(self):
        ...
        for datatype in datatypes:
            if datatype in self.events_file.keys():
                ...
                # for prex in ['ori', 'down2', 'down4', 'down8']:
                for prex in ['ori']:
                    s = 0
                    added = 0
                    for image in self.events_file[datatype]:
