...
class YAMLParser:
    ...
    def __init__(self, config, args, modification=None):
        ...
        self.args = args
        config_runid = self.config.get('id', None)
        parse_runid = args.runid
        assert config_runid is not None or parse_runid is not None, 'Please input runid!'
        if config_runid is not None:
            run_id = config_runid
        else:
            run_id = parse_runid
        ...
        self.log_config(self.config, self.log_dir / 'config.yml')
