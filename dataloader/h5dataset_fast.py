...
class H5Dataset(Dataset):
    ...
    def process_seq(self):
        self.SeqLatentFPreLoad = []
        self.SeqBlurryFPreLoad = []
        self.SeqHREvPreLoad = []
        self.LatentTsPreLoad = []
        self.RelativeLatentTsPreLoad = []
        self.BlurryTsPreLoad = []
        self.SeqExposureDutyPreLoad = []

        for index in range(len(self.SeqIndices)):
            ...
            for LoadIndex in Sequence:
                ...
                LatentFrames = torch.stack(LatentFramesList, dim=0) # NxN1x3xHxW 
                BlurryFrames = torch.stack(BlurryFramesList, dim=0) # Nx3xHxW
                ...
                ExposureDuty = torch.stack(ExposureDutyList).unsqueeze(-1) # Nx1
            ...
            SeqLatentF = torch.stack(SeqLatentFList, dim=0) # LxNxN1x3xHxW 
            SeqBlurryF = torch.stack(SeqBlurryFList, dim=0) # LxNx3xHxW
            ...
            self.SeqLatentFPreLoad.append(SeqLatentF)
            self.SeqBlurryFPreLoad.append(SeqBlurryF)
            self.SeqHREvPreLoad.append(SeqHREv)
            self.LatentTsPreLoad.append(LatentTs)
            self.RelativeLatentTsPreLoad.append(RelativeLatentTs)
            self.BlurryTsPreLoad.append(BlurryTs)
            self.SeqExposureDutyPreLoad.append(SeqExposureDuty)
    ...
    def __getitem__(self, index, seed=None):
        ...
        SeqLatentF = self.SeqLatentFPreLoad[index] # LxNxN1x3xHxW 
        SeqBlurryF = self.SeqBlurryFPreLoad[index] # LxNx3xHxW
        # SeqLREv = torch.stack(SeqLREvList, dim=0) # LxTBx2xH1xW1
        # SeqLREvBi = torch.stack(SeqLREvBiList, dim=0) # LxTBx2xHxW
        SeqHREv = self.SeqHREvPreLoad[index] # LxTBx2xHxW
        LatentTs = self.LatentTsPreLoad[index] # LxNumF
        RelativeLatentTs = self.RelativeLatentTsPreLoad[index] # LxNumPxNumF
        BlurryTs = self.BlurryTsPreLoad[index] # LxNumPx2
        SeqExposureDuty = self.SeqExposureDutyPreLoad[index] # LxNumPx1
...
if __name__ == '__main__':
    ...
    path_to_config = '/data2/wengwm/work/code/EVFI-BE/config/train_ledvdi.yml'
