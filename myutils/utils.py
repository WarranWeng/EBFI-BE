from datalist.generate_datalist import write_txt
import os
import datetime
...
def Frame2DCP(ims: torch.tensor, sz=35):
    '''
    im: torch.tensor, Bx3xHxW
    
    return: dark, torch.tensor, Bx1xHxW
    '''
    darkList = []
    for i in range(ims.size(0)):
        im = ims[i].permute(1, 2, 0).cpu().numpy() # HxWx3 
        b, g, r = cv2.split(im)
        dc = cv2.min(cv2.min(r, g), b)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (sz, sz))
        dark = cv2.erode(dc, kernel)
        darkList.append(torch.from_numpy(dark).unsqueeze(0))
    darkTensor = torch.stack(darkList, dim=0).cuda() # Bx1xHxW

    return darkTensor
    

def Frame2Lap(ims: torch.tensor):
    '''
    im: torch.tensor, Bx3xHxW
    ...
    for i in range(ims.size(0)):
        ...
        lap = cv2.Laplacian(gray, cv2.CV_64F).astype('float32')
        LapList.append(torch.from_numpy(lap).unsqueeze(0))
    LapTensor = torch.stack(LapList, dim=0).cuda() # Bx1xHxW
...
class Resumer:
    ...
    def resume_model(self, model, key):
        ...
        model.module.load_state_dict(self.checkpoint[key]['states'], strict=False)
