...
from dataloader.h5dataloader import HDF5DataLoader, HDF5DataLoaderRealData
from loss import *
from models.Ours.model_singleframe import ExposureDecision, ExposureDecisionNoEvents
...
class Trainer:
    def __init__(self, args):
        ...
        self.BlurryFashion = self.config_parser['model']['BlurryFashion']
    ...
    def iteration_based_training(self):
        ...
        while True:
            ...
            for idx_epoch, inputs_seq in enumerate(self.train_dataloader):
                ...
                L, B, NumP = SeqBlurryF.size()[:3]
                for idxL in range(L):
                    if stop_training or complete_training:
                        break
                    LoadBlurryF = SeqBlurryF[idxL] # BxNumPx3xHxW
                    HREv = SeqHREv[idxL] # BxTBx2xHxW
                    HREvNew = HREv.view(HREv.size(0), -1, HREv.size(3), HREv.size(4)) # Bx(TBx2)xHxW
                    LoadExposureDuty = SeqExposureDuty[idxL] # BxNumPx1

                    for i in range(NumP):
                        BlurryF = LoadBlurryF[:, i] # Bx3xHxW
                        ExposureDuty = LoadExposureDuty[:, i] # Bx1

                        if self.BlurryFashion == 'DarkCh':
                            BlurryLevel = Frame2DCP(BlurryF) # Bx1xHxW
                        elif self.BlurryFashion == 'Lap':
                            BlurryLevel = Frame2Lap(BlurryF) # Bx1xHxW
                        elif self.BlurryFashion == 'RGB':
                            BlurryLevel = BlurryF # Bx3xHxW

                        elif self.BlurryFashion == 'RGBDark':
                            Dark = Frame2DCP(BlurryF) # Bx1xHxW
                            BlurryLevel = torch.cat([BlurryF, Dark], dim=1) # Bx4xHxW
                        elif self.BlurryFashion == 'RGBLap':
                            Lap = Frame2Lap(BlurryF) # Bx1xHxW
                            BlurryLevel = torch.cat([BlurryF, Lap], dim=1) # Bx4xHxW
                        else:
                            raise Exception('Wrong blurry convertion fashion!!')

                        with self.model.no_sync():
                            Ex = self.model(HREvNew, BlurryLevel) # Bx1

                            loss = self.loss['mse'](Ex, ExposureDuty) / accu_step
                            loss.backward()
        ...
        for batch_idx, inputs_seq in enumerate(self.valid_dataloader):
            ...
            for idxL in range(L):
                ...
                for i in range(NumP):
                    ...
                    Ex = self.model(HREvNew, BlurryLevel) # Bx1
                    loss += self.loss['mse'](Ex, ExposureDuty)
...
def main(config_parser):
    ...
    train_dataloader = HDF5DataLoaderRealData(config['train_dataloader'])
    valid_dataloader = HDF5DataLoaderRealData(config['valid_dataloader'])
